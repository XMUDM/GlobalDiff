...
def evaluate(model, data_loder, diff, device,epoch_index,is_save):
    ...
    if is_save:
        save_metrics("./results/encoder_20.csv",epoch_index,hr_list,ndcg_list,topk)
    ...
    print('{:<10.6f} {:<10.6f} {:<10.6f} {:<10.6f} {:<10.6f} {:<10.6f}'.format(hr_list[0], (ndcg_list[0]), hr_list[1], (ndcg_list[1]), hr_list[2], (ndcg_list[2])))
    if is_save:
        save_metrics("./results/diffusion_20.csv",epoch_index,hr_list,ndcg_list,topk)
    ...
    print('{:<10.6f} {:<10.6f} {:<10.6f} {:<10.6f} {:<10.6f} {:<10.6f}'.format(hr_list[0], (ndcg_list[0]), hr_list[1], (ndcg_list[1]), hr_list[2], (ndcg_list[2])))
    if is_save:
        save_metrics("./results/fusion_20.csv",epoch_index,hr_list,ndcg_list,topk)
...
if __name__ == '__main__':
    ...
    ##########################日志#######################################
    #日志文件名按照程序运行时间设置
    log_file_name =  './Log/log-'+args.dataset_code +'-GlobalDiff_bert-' +time.strftime("%Y%m%d-%H%M%S", time.localtime()) + '.log'
    ...
    best_model = torch.load('/data1/mxluo/mxluo/GlobalDiff/pretrain_models/Bert/BERTMl1m.pth').get('model_state_dict')
    BERT_model.load_state_dict(best_model)

    model = Tenc(args,device,BERT_model)

    # model.load_state_dict(torch.load ('/data1/mxluo/mxluo/GlobalDiff/experiments/GlobalDiff_kuaishou_bert_leave5out.pth'))
