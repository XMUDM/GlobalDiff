...
if __name__ == '__main__':
    ...
    best_model = torch.load('/data1/mxluo/mxluo/GlobalDiff/pretrain_models/Bert/CBITBeauty.pth').get('model_state_dict')
    ...
    param_size = sum(p.numel() * p.element_size() for p in model.bert.parameters())  
    buffer_size = sum(b.numel() * b.element_size() for b in model.bert.buffers())  # 所有buffer的字节数
    total_size = (param_size + buffer_size) / (1024 ** 2)  # 转换为 MB
    print(f"Total parameters: {total_params}")
    print(f"Model size: {total_size:.2f} MB")

    total_params = sum(p.numel() for p in model.bert_out.parameters())
    param_size = sum(p.numel() * p.element_size() for p in model.bert_out.parameters())  
    buffer_size = sum(b.numel() * b.element_size() for b in model.bert_out.buffers())  # 所有buffer的字节数
    total_size = (param_size + buffer_size) / (1024 ** 2)  # 转换为 MB
    print(f"Total parameters: {total_params}")
    print(f"Model size: {total_size:.2f} MB")
    ...
    # model.load_state_dict(torch.load ('/data1/mxluo/mxluo/GlobalDiff/experiments/GlobalDiff_beauty_cbit_leave5out.pth'))
