...
def parse_args():
    ...
    parser.add_argument('--hidden_size', type=int, default=128,help='Number of hidden factors, i.e., embedding size.')
    #######################CBIT###########################
...
class Tenc(nn.Module):
    ...
    def cacu_seq(self, seqs):
        ...
        """
        logits,inputs_encoding = self.SASRec.predictAll(seqs)
...
def evaluate(model, data_loder, diff, device,epoch_index,is_save):
    ...
    if is_save:
        save_metrics("./results/encoder_mse.csv",epoch_index,hr_list,ndcg_list,topk)
    ...
    print('{:<10.6f} {:<10.6f} {:<10.6f} {:<10.6f} {:<10.6f} {:<10.6f}'.format(hr_list[0], (ndcg_list[0]), hr_list[1], (ndcg_list[1]), hr_list[2], (ndcg_list[2])))
    if is_save:
        save_metrics("./results/diffusion_mse.csv",epoch_index,hr_list,ndcg_list,topk)
    ...
    print('{:<10.6f} {:<10.6f} {:<10.6f} {:<10.6f} {:<10.6f} {:<10.6f}'.format(hr_list[0], (ndcg_list[0]), hr_list[1], (ndcg_list[1]), hr_list[2], (ndcg_list[2])))
    if is_save:
        save_metrics("./results/fusion_mse.csv",epoch_index,hr_list,ndcg_list,topk)
...
if __name__ == '__main__':
    ...
    ##########################日志#######################################
    #日志文件名按照程序运行时间设置
    log_file_name =  './Log/log-'+args.dataset_code +'-GlobalDiff-SasRec' +time.strftime("%Y%m%d-%H%M%S", time.localtime()) + '.log'
    ...
    SAS_model.load_state_dict(torch.load('./pretrain_models/SasRec/ml1m_leave5out.pth', map_location=device))
    # SAS_model.load_state_dict(best_model)
    SAS_model.to(device)
    model = Tenc(args,device,SAS_model)
    model.load_state_dict(torch.load ('/data1/mxluo/mxluo/GlobalDiff/experiments/GlobalDiff_ml1m_sasrec_leave5out.pth'))



    total_params = sum(p.numel() for p in model.SASRec.parameters())
    param_size = sum(p.numel() * p.element_size() for p in model.SASRec.parameters())  
    buffer_size = sum(b.numel() * b.element_size() for b in model.SASRec.buffers())  # 所有buffer的字节数
    ...
    param_size = sum(p.numel() * p.element_size() for p in model.unet.parameters())  
    buffer_size = sum(b.numel() * b.element_size() for b in model.unet.buffers())  # 所有buffer的字节数
    total_size = (param_size + buffer_size) / (1024 ** 2)  # 转换为 MB
    print(f"Total parameters: {total_params}")
    print(f"Model size: {total_size:.2f} MB")
    ...
    for i in range(args.epoch):
        ...
        if(i<args.pretrain_epoch):
            for batch_idx, batch in enumerate(tqdm_dataloader):
                ...
                optimizer.zero_grad()
                # 这里简单用celoss了,也可以使用原模型的训练得到ckpt
                loss = 0
            ...
            ############################冻结pretrain模型的参数##################################
            for name, param in model.SASRec.named_parameters():
