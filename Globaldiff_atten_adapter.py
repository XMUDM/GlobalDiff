...
class Tenc(nn.Module):
    def __init__(self, args,device,InitModel):
        ...
        #################################bert############################################
        self.bert = InitModel.bert
        # 对于历史序列添加一层linear用来做交叉熵损失
        self.bert_out =InitModel.out
        ...
        self.atten4voting = nn.Linear((self.item_num+1)*2, 1, bias=False)
    def forward(self, x, seqs, step,random_indice,pretrain=False):
        """
            args
                ...
                seqs (torch.tensor (B,S): 历史序列的token id,
        ...
        #####################cross-attention#############################
        x= F.normalize(x)
        inputs_encoding,seqs_logits=self.cacu_seq(seqs)
        x=self.w1(x)        
        # (B,D)
        target_encoding = inputs_encoding [torch.arange(B), random_indice, :]
        #为了防止表征过强，这里进行归一化和dropout
        # target_encoding = F.normalize(target_encoding)
        # target_encoding = self.drop(target_encoding)
        # (B,D)
        global_encoding = inputs_encoding.mean(dim=1)
        # #(B,1,2*D)
        ...
        ###################################通过线性层计算注意力分数，形状为 (B, 1)################################################

        seqs_target_logits= seqs_logits.view(B,seqs.shape[1],-1)[torch.arange(B), random_indice, :]
        # (B,2*V)
        cat_scores=torch.cat((diff_logits,seqs_target_logits),dim=-1)
        attn_scores = self.atten4voting(cat_scores)
        # 应用 softmax 函数计算权重，形状为 (B, 1)
        attn_weights = F.softmax(attn_scores, dim=1)
        # 重新加权
        # 加权求和
        fusion_logits = diff_logits* attn_weights+ seqs_target_logits*(1-attn_weights)
        predicted_x = fusion_logits.unsqueeze(1)
        return predicted_x,seqs_logits,fusion_logits,inputs_encoding
    ...
    def cacu_seq(self, seqs):
        ...
        # logits = self.bert_out(self.out_before(inputs_encoding))
    ...
    def predict(self, states,target, diff,mask_indice):
        ...
        """
        with torch.no_grad():
            seqs_encoding,seqs_logits=self.cacu_seq(states)
...
def evaluate(model, data_loder, diff, device,epoch_index,is_save):
    ...
    if is_save:
        save_metrics("./results/encoder_20_ours1.csv",epoch_index,hr_list,ndcg_list,topk)
    ...
    print('{:<10.6f} {:<10.6f} {:<10.6f} {:<10.6f} {:<10.6f} {:<10.6f}'.format(hr_list[0], (ndcg_list[0]), hr_list[1], (ndcg_list[1]), hr_list[2], (ndcg_list[2])))
    if is_save:
        save_metrics("./results/diffusion_20_ours1.csv",epoch_index,hr_list,ndcg_list,topk)
    ...
    print('{:<10.6f} {:<10.6f} {:<10.6f} {:<10.6f} {:<10.6f} {:<10.6f}'.format(hr_list[0], (ndcg_list[0]), hr_list[1], (ndcg_list[1]), hr_list[2], (ndcg_list[2])))
    if is_save:
        save_metrics("./results/fusion_20_ours1.csv",epoch_index,hr_list,ndcg_list,topk)
...
if __name__ == '__main__':
    ...
    ##########################日志#######################################
    #日志文件名按照程序运行时间设置
    log_file_name =  './Log/log-'+args.dataset_code +'-GlobalDiff-atten-adapter' +time.strftime("%Y%m%d-%H%M%S", time.localtime()) + '.log'
    ...
    ##########################加载训练好的CBIT模型##################################################################
    CBIT_model = BERTModel(args)

    best_model = torch.load('/data1/mxluo/mxluo/GlobalDiff/pretrain_models/Bert/CBITMl1m.pth').get('model_state_dict')
    CBIT_model.load_state_dict(best_model)

    model = Tenc(args,device,CBIT_model)
    ...
    for i in range(args.epoch):
        ...
        if(i<args.pretrain_epoch):
            for batch_idx, batch in enumerate(tqdm_dataloader):
                ...
                # # 添加bert的loss
                # loss = 0
                # seqs_encoding,seqs_logits=model.cacu_seq(pos_tokens[:,0,:]) 
                # #编码器的交叉熵损失
                # curr_labels=pos_labels[:,0,:].clone().view(-1)
                # loss+=celoss_function(seqs_logits.to(device),curr_labels)
            ...
            for param in model.bert_out.parameters():
                param.requires_grad = False
